...
fun date(date: String): Boolean {
    ...
    return date.matches(pattern.toRegex()) // matches проверяет соответствует ли строка выражению
}
...
//    val regexp = """<[^/][a-z_0-9]*>?"""
    val regexp = """<[^>/]+>?"""
    val found = regexp.toRegex().findAll(html) // findAll найти все варианты совпадения по шаблону
...
fun genPhone() {
    ...
    phoneTemplate.forEach { it -> println(it) }
